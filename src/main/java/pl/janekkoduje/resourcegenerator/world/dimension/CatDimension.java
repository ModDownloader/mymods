...
import pl.janekkoduje.resourcegenerator.procedures.CatPlayerEntersDimensionProcedure;
import pl.janekkoduje.resourcegenerator.item.CatItem;
...
public class CatDimension extends ResourceGeneratorModElements.ModElement {
	@ObjectHolder("resource_generator:cat")
	public static final ModDimension dimension = null;
	@ObjectHolder("resource_generator:cat_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public CatDimension(ResourceGeneratorModElements instance) {
		super(instance, 9);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("cat"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("resource_generator:cat")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("resource_generator:cat"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("resource_generator:cat"));
	}
	...
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("taiga")),
				...
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("resource_generator:mybiome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new CatItem().setRegistryName("cat"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		...
		private TeleporterDimensionMod getTeleporterForDimension(Entity entity, BlockPos pos, ServerWorld nextWorld) {
			BlockPattern.PatternHelper bph = CatDimension.CustomPortalBlock.createPatternHelper(entity.world, pos);
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("cat_portal", Vec3i::compareTo, 300);
	@SubscribeEvent
	public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
		try {
			...
			poi = (PointOfInterestType) method.invoke(null, "cat_portal",
	...
	public static class TeleporterDimensionMod implements ITeleporter {
		...
		public BlockPattern.PortalInfo placeInExistingPortal(BlockPos p_222272_1_, Vec3d p_222272_2_, Direction directionIn, double p_222272_4_,
			...
			return optional.map((p_226707_7_) -> {
				...
				BlockPattern.PatternHelper blockpattern$patternhelper = CatDimension.CustomPortalBlock.createPatternHelper(this.world, blockpos);
		...
		if (event.getTo() == type) {
			{
				...
				CatPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(Blocks.TNT.getDefaultState().getBlock(),
